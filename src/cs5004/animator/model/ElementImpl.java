...
public class ElementImpl implements Element {
     ...
     */
    public ElementImpl(String id, Shape shape, Position position, Color color,
                       Size size, int appearTime, int disappearTime) {
        this.id = id;
        this.shape = shape;
    ...
    private Size getSizeByTic(int time) {
        ...
        for (Transformation t : scaleSchedule) {
            if (time >= t.getToTime()) {
                ...
                double percentage = (double) (time - t.getFromTime()) / (t.getToTime() - t.getFromTime());
                int one = (int) (curr.getFirstArg() + (tmp.getFirstArg() - curr.getFirstArg()) * percentage);
                int two = (int) (curr.getSecondArg() + (tmp.getSecondArg() - curr.getSecondArg()) * percentage);
    ...
    /**
     * This method is a helper to generate text log and svg log.
     * This method is new added.
     *
    ...
    private void changeLogs(int speed) {
        ...
        for (Transformation t : moveSchedule) {
            ...
            String svg = animationSvgGenerator(shape.getSvgPosTitle1(), curr1.getX(), tmp.getX(),
                    ((double) t.getFromTime()) / speed, ((double) t.getToTime()) / speed) +
                    animationSvgGenerator(shape.getSvgPosTitle2(), curr1.getY(), tmp.getY(),
                            ((double) t.getFromTime()) / speed, ((double) t.getToTime()) / speed);
            svgMotion += svg;
            curr1 = tmp;
        }
        ...
        for (Transformation t : colorSchedule) {
            ...
            log.add(new LogNode(t.getFromTime(), s));
            String svg = String.format("<animate attributeName=\"fill\" from=\"%s\" " +
                            "to=\"%s\" begin=\"%.1fs\" end=\"%.1fs\" dur=\"%.1f\" fill=\"freeze\"/>\n",
                    curr2.toSvgString(), tmp.toSvgString(), ((double) t.getFromTime()) / speed,
        ...
        for (Transformation t : scaleSchedule) {
            ...
            String s = String.format("Shape %s scales from %s: %.1f, %s: %.1f to" +
                            " %s: %.1f, %s: %.1f from t=%d to t=%d\n", id, shape.getSizeTitle1(),
                    curr3.getFirstArg(), shape.getSizeTitle2(), curr3.getSecondArg(),
                    shape.getSizeTitle1(), tmp.getFirstArg(), shape.getSizeTitle2(),
                    tmp.getSecondArg(), t.getFromTime(), t.getToTime());
            log.add(new LogNode(t.getFromTime(), s));
            String svg = animationSvgGenerator(shape.getSvgSizeTitle1(), curr3.getFirstArg(),
                    tmp.getSecondArg(), ((double) t.getFromTime()) / speed,
                    ((double) t.getToTime()) / speed) +
                    animationSvgGenerator(shape.getSvgSizeTitle2(), curr3.getFirstArg(),
                            tmp.getSecondArg(), ((double) t.getFromTime()) / speed,
                            ((double) t.getToTime()) / speed);
            svgMotion += svg;
            curr3 = tmp;
    ...
    public String svgShape(int speed) {
        ...
        sb.append(SvgHelper.assignment(Integer.toString(position.getX()),
                shape.getSvgPosTitle1()) + " ");
        sb.append(SvgHelper.assignment(Integer.toString(position.getY()),
                shape.getSvgPosTitle2()) + " ");
        sb.append(SvgHelper.assignment(Integer.toString(size.getFirstArg()),
                shape.getSvgSizeTitle1()) + " ");
        sb.append(SvgHelper.assignment(Integer.toString(size.getSecondArg()),
        ...
        sb.append(">\n");
        sb.append(String.format("<animate attributeName=\"visibility\" " +
                "to=\"visible\" begin=\"%ds\"/>\n", appearTime / speed));
        sb.append(String.format("<animate attributeName=\"visibility\" " +
                "to=\"hidden\" begin=\"%ds\"/>\n", disappearTime / speed));
