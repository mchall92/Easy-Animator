...
public final class Builder implements AnimationBuilder<Window> {
  ...
  /**
   * Construct three collections to record motion information of three different motions. Also
   * records all information of transformation in a hashmap.
   */
  public Builder() {
  ...
  /**
   * This method helps to convert a string representation of a shape to a shape enum(ignoring case).
   *
  ...
  public Window build() {
    ...
    for (TransMove move : transMoves) {
      window.move(move.getName(), move.getX(), move.getY(), move.getFromTime(), move.getToTime());
    }
  ...
  @Override
  public AnimationBuilder<Window> addMotion(
      String name,
      int t1,
      int x1,
      int y1,
      ...
      int g2,
      int b2) {
    this.attributeSrc.get(name).getLs().add(new TransAll(name, t1, t2, x1, y1, r1, g1, b1, w1, h1));
    if (x1 != x2 || y1 != y2) {
